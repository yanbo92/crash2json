...
class ExceptionBacktrace:
    ...
    def get_threads_list(self):
        ...
        for line in lines:
            if crash_thread_flag and "Thread 0 crashed" in line:
                # 停止存储堆栈
                break

            if "Exception Backtrace:" in line:
    ...
    def get_crash_thread_info(self):
        """
        从.crash文件中获取崩溃线程信息：
        :return: 结果 dict
        """
        ...
        for f in threads_list:
            if "Exception Backtrace:" in f:
                crash_thread += f
        return self.get_thread_info(crash_thread)

    def get_thread_info(self, thread):

        # 初始化
        thread_info_dict = {"name": "", "stackFrames": []}
        ...
        for line in thread_lines:
            if "name:" in line:
                ...
                thread_info_dict["name"] = strs[1]
            else:
                if "0x" in line:
                    thread_info_dict["stackFrames"].append(self.get_stack_frame_info(line))
    ...
    def toJson(self, filename=""):
        if filename:
            ...
            json_name = json_name + "-exception_backtrace"
